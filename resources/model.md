- Manufacturing Device
    * Lacquering, Enameling & Plastic Coating Systems
    * Plastic Assembly Equipment
    * Bearing Production and Machining Equipment
    * Electroplating Units
    * Miniature Machines for Training
    * Plastic Joining Equipment
    * Systems for Industrial Plasma & Surface Technology
    * Composite Working Machines (Tape Laying, Filament Winding, etc.)
    * Heat Treating Equipment and Supplies, Including Ovens and Furnaces
    * Manufacturing Equipment for Automation & Electrical Systems
    * Manufacturing Equipment for Mechanical Transmission Systems, Motors and Seals
    * Plastic Cutting and Slitting Equipment
    * Plastic Injection Molding Machines
    * Plastic-Working Equipment
    * Plastic Molding and Forming Equipment
    * Stress Relief Equipment
    * Surface Treatment and Coating Equipment
    * Woodworking Equipment
    * Plastic Film Sealing Equipment
    - Turning Center
        * Turning Centers, Horizontal, CNC
        * Turning Centers, Inverted Vertical, CNC
        * Turning Centers, Twin-Spindle/Twin-Turret
        * Turning Centers, Vertical, CNC
    - Lathe
        - Horizontal Lathe CNC
            * Lathes, Horizontal, CNC
            * Lathes, Horizontal, CNC-Swiss Style Lathes
            * Lathes, Horizontal, CNC-Multi-Spindle
            * Lathes, Horizontal, CNC-Twin Spindle Machines
        * Lathes, Vertical, CNC
        * Lathes, Cut-Off
        * Lathes, Manual
    - Machining Center
        - Horizontal Machining Center
            * Machining Centers, Horizontal
            * Machining Centers, Horizontal Spindle (Up to Four-Axis)
            * Machining Centers, Horizontal Spindle, Five-Axis
        - Vertical Machining Center
            * Machining Centers, Vertical Column
            * Machining Centers, Vertical Bridge & Gantry
            * Machining Centers, Vertical Spindle (Up to Four-Axis)
            * Machining Centers, Vertical Spindle, Five-Axis
        * Machining Centers, Universal
    - Grinding Machine
        - Surface Grinding Machine
            * Grinding Machines, Surface
            * Grinding Machines, Surface, Reciprocal Table
            * Grinding Machines, Surface, Rotary Table
        * Grinding Machines, Abrasive Belt
        * Grinding Machines, Centerless
        * Grinding Machines, Creep Feed
        * Grinding Machines, Cylindrical OD
        * Grinding Machines, Disc, Single or Double
        * Grinding Machines, Internal Cylindrical
        * Grinding Machines, Jig
        * Grinding Machines, Profile
        * Grinding Machines, Thread
        * Grinding Machines, Tool, Cutter and Drill Point
        * Grinding Machines, Universal (ID/OD)
    - Welding Machine
        * Welding Machines and Equipment
        * Welding Machines, Electron-Beam
        * Welding Machines, Friction
        * Welding Machines, Gas
        * Welding Machines, Electric
        * Welding Machines, Laser
        * Welding Machines, Systems or Robotic
    - EDM Machine
        * EDM, Small Hole
        * EDM, Wire Type
        - Ram Type EDM Machine
            * EDM, Ram Type
            * EDM, Ram Type (Die-Sinking) CNC
            * EDM, Ram Type (Die-Sinking) Manual
    - Honing Machine
        * Honing Machines
        * Honing Machines, Horizontal Spindle
        * Honing Machines, Multiple Spindle
        * Honing Machines, Vertical Spindle
    - Press
        - Hydraulic Press
            * Presses, Hydraulic
            * Presses, Hydraulic, Gap Frame
            * Presses, Hydraulic, Vertical Straight Side
        - Punch Press
            * Presses, Punch
            * Punch Press/Laser (or Plasma), CNC Combination
            * Punch Presses, CNC Turret
        - Mechanical Press
            * Presses, Mechanical
            * Presses, Mechanical, Gap Frame
            * Presses, Mechanical, Vertical Straight Side
        * Presses, Pneumatic
        * Presses, Fine Blanking
        * Presses, Stamping, Automatic
        * Presses, Straightening
    - Additive Machine
        * Sheet Lamination
        * Directed Energy Deposition (DED)
        * Hybrid - Additive Manufacturing
        * Powder Bed Fusion (PBF)
        * Vat Photopolymerization
        * Material Jetting
        * Binder Jetting
        * Material Extrusion
        * Mold - Enabled Additive Manufacturing
    - Broaching Machine
        * Broaching Machines, Continuous
        * Broaching Machines, Horizontal
        * Broaching Machines, Internal
        * Broaching Machines, Vertical
        * Shaping and Slotting Machines
    * Metal Disintegrators
    * Electro-Chemical Machines (ECM)
    * Laser Cutting Systems
    * Laser Marking Systems
    * Laser Welding and Cladding Machines
    * Lasers, CO2
    * Lasers, Nd: YAG, Continuous Wave
    * Lasers, Nd: YAG, Pulsed
    * Marking and Engraving Machines
    * Plasma-Arc/Plasma Cutting Machines
    * Water Jet Cutting Machines
    - Gear Manufacturing Machine
        * Gear Deburring Machines
        * Gear Generation (Bevel) Machines
        * Gear Grinding Machines
        * Gear Hobbing Machines
        * Gear Honing Machines
        * Gear Inspection Machines
        * Gear Lapping Machines
        * Gear Shaping Machines
        * Gear Shaving Machines
        * Gear Rolling Machines
    * Coating Equipment
    * Abrasive Flow Finishing Machines
    * Buffing and Polishing Machines
    * Centrifugal Barrel and Disc Finishing Machines
    * Deburring Equipment
    * Lapping Machines
    * Roll Finishing Machines
    * Super (Micro) Finishing Machines
    * Superabrasive Machining Systems
    * Vibratory Tumbling Machines
    - Boring Machine
        * Boring Machines
        * Boring Machines, Jig
    - Drilling Machine
        * Drilling and Tapping Machines, CNC
        * Drilling and Tapping Units, Fixtured
        * Drilling Machines, Bench and Column
        * Drilling Machines, Deep Hole (Gundrilling)
        * Drilling Machines, Multi-Spindle
        * Drilling Machines, Radial Arm
    - Milling Machine
        * Milling Machines, Bed-Type
        * Milling Machines, Graphite
        * Milling Machines, Knee and Column, Non-ATC
        * Milling Machines, Planer, Gantry and Bridge Type
        * Milling Machines, Profile and Duplicating
        * Milling Machines, Universal
    * Nano & Micro Machining Equipment
    * Slotting Saws
    * Abrasive Cut-Off Machines
    - Bandsaw    
        * Bandsaws
        * Bandsaws, Vertical Tilt Frame
    * Hacksaws
    - Sawing Machine
        * Sawing Machines, Circular Cut-Off Saws (Cold)
        * Sawing Machines, Contour
        * Sawing Machines, Other
    - Screw Machine
        * Screw Machines, Multi-Spindle, Cam-Type
        * Screw Machine Attachments, Cams & Related Equipment
        * Screw Machines, Single-Spindle, Cam-Type
        * Screw Machines, Multi-Spindle, CNC
        * Screw Machines, Single-Spindle, CNC
        * Screw Machines, Swiss, CNC
        * Screw Machines, Swiss, Cam-Type
    * End Finishing Equipment, Bar and Tube
    * End Turning Machines
    * Thread Cutting Machines
    * Thread Rolling Machines
    * Threading and Cut-Off Machines, Pipe and Bar
    * Turn/Mill Machines, CNC
    * Ultrasonic Machining Equipment
    * Coil Slitting Equipment
    * Diecutting Machines
    * Stretch Benders
    * Strip Handling Machines
    * Coil Equipment Systems
    * Cold Forming, Cold Heading Machines
    * Cold Forming, Wire Forming Machines
    * Insertion Equipment - Fastener
    * Vacuum Forming Machines
    * Cut to Length Machines
    * Die Handling Equipment
    * Foundry Equipment
    * Hot Forming, Forging Machines
    * Iron Workers
    * Notching and Slotting Machines
    * Pipe and Tube Bending Machines
    * Pipe Fabricating and Forming Equipment
    * Press Brakes
    * Profile Rolling Machines
    * Roll Forming and Bending Machines
    * Shears, Billet, Bar, Rod and Angle
    * Shears, Guillotine (Squaring)
    * Straightening Machines
    * Wire Forming Machines
    * Coil Winding Machines
    * Cold Forming, Upsetters
    * Hydro-Forming Machines
    * Spline Rolling Machines
    * Cold Forming, Riveting Machines
    * Diecasting Machines
    * Spin-Forming Machines
    * Swaging Machines
    * Autocollimators
    * Balancing Machines
    * Borescopes
    * Calibration Equipment
    * Comparators, Optical and Other
    * Coordinate Measuring Machines
    * Coordinate Measuring Machines, Portable
    * Data Collection Devices for Gaging, SPC, etc.
    * Flatness Measuring Equipment
    * Flaw Detection Equipment
    * Gages, Electronic/Digital
    * Gages, Mechanical
    * Gear Inspection Equipment
    * Hardness Testing Equipment
    * Interferometers
    * Laser Measurement Systems
    * Leak Testing Equipment
    * Material Testing & Analysis Equipment
    * Probes, Electromechanical
    * Refractometers
    * Roundness Measuring Equipment
    * Surface Finish Measuring Equipment
    * Surface Plates
    * Vision Systems
    * Weight Scales
    * Deburring, Etching, Cleaning & Peening
    * Shearing
    * Coil Handling
    - Material Handling System
        - Transfer Machine
            * Transfer Machines, In-line
            * Transfer Machines, Dial Index
            * Transfer Machines, Rotary
            * Transfer Machines, Shuttle-type
            * Transfer Machines, Trunnion
        * Automated Guided Vehicles
        * Automatic Storage/Retrieval Systems
        * Bar Feed Mechanisms
        * Chip Conveyors and Handling Equipment
        * Conveyors & Other Part Handling Automation
        * Gantry Loaders
        * Lifting and Positioning Equipment
        * Pallet Changers/Shuttles and Programmable Transfer Devices
        * Press Automation Devices
        * Press Feeds and Loading Equipment
        * Workpiece Handling, Loading and Feeding Equipment
        * Pallet Changers - Manual
        * Bowl Feeders
        * Mobile Robots - (AGV / UGV / AMR )
        * Multi-Machine Tool-Storage Systems
        * Forming Equipment-Loading & Positioning
    * Laser Technology
    * Stationary Robots - (Robot Arm)
    - Sensing and Inspection System
        * Measuring and Test Systems
        * Industrial Image Processing Systems
        * Industrial Automatic Data Capturing and Identification Systems
        * Sensors and Actuators
        * Autocollimators
        * Balancing Machines
        * Borescopes
        * Calibration Equipment
        * Comparators, Optical and Other
        * Coordinate Measuring Machines
        * Coordinate Measuring Machines, Portable
        * Data Collection Devices for Gaging, SPC, etc.
        * Flatness Measuring Equipment
        * Flaw Detection Equipment
        * Gages, Electronic/Digital
        * Gages, Mechanical
        * Gear Inspection Equipment
        * Hardness Testing Equipment
        * Interferometers
        * Laser Measurement Systems
        * Leak Testing Equipment
        * Material Testing & Analysis Equipment
        * Probes, Electromechanical
        * Refractometers
        * Roundness Measuring Equipment
        * Surface Finish Measuring Equipment
        * Surface Plates
        * Vision Systems
        * Weight Scales
    - Control System
        * Industrial PCs
        * Process Automation Systems & Services
        * Programmable Logic Controls (PLC)
        * Control Systems, PLC, SCADA Systems
        * Embedded Systems
        * Computer Numerical Control (CNC) Software (sold independent of hardware)
        * Computer Numerical Control (CNC) Units
        * Digital Readout (DRO) Units
        * Direct or Distributive Numerical Control (DNC) Systems
        * Robot Controllers
- Infrastructure Device
    * Air Compressors
    * Coolant Delivery Systems
    * Coolant Reclamation Equipment
    * Coolants, Cutting Fluids and Oils
    * Dust Collectors, Air Filtration and Treatment Equipment
    * Environmental and Pollution Control Equipment
    * Oil Filtration and Treatment Equipment
    * Noise Abatement
    * Vacuum Pumps
    * Vacuum Technology
    * Water Filtration and Treatment Equipment
    * Chip and Sludge Dragouts
    * Cleaning & Degreasing Equipment, Aqueous
    * Cleaning and Degreasing Equipment
    * Degreasing Equipment, Vapor
    * Furnishings, Shop and Office (files, desks, benches, cabinets, tool chests, drawing boards & lighting)
    * Solvents and Degreasing Agents
    * Compressed Air Processing Equipment, Dryers, Coolers, etc.
    * Compressor Accessories & Components (Valves, Pressure Gauges & Reducers, Silencers, Heat Recovery & Cooling Systems)
    * Compressed Air Equipment; Other
    * Vacuum Fittings, Seals, Valves & Accessories
    * Condition Monitoring and Vibration Reduction
    * Plant Maintenance and Repair Equipment
    * Safety Related Products
    * Industrial Building Automation Systems
    * Safety and Security Systems
    * Communications Networks, and Data Transmission Equipment
    * Communication, Networks and Field Bus Systems
- Production Cell
    * Automated System & Cells for Assembly & Test
    * Automated System & Cells for Material Forming & Fabricating
    * Automated System & Cells for Material Joining
    * Automated System & Cells for Material Removal
    * Automated Pallet Systems
    * Machining Cells & FMS
- Software Artifact
    - Digital Artifact
        - CAD/CAM
            * Software, CAD/CAM
            * Printed Circuit Board Design and Manufacturing
            * Software, Engineering Analysis and Simulation
            * Software, Nesting
            * Software, Post Processors, Stand-Alone (sold independent of CAM system)
            * Software, CAD/CAM for Additive Manufacturing
        * Software  ( for Drives, Gears Hydraulics, Pneumatics)
        * Software, Accounting/General Ledger/Payroll
        * Software, Calibration
        * Software, Computer Aided Facility Management Systems
        * Software, Customer Relationship Management Software, CRM
        * Software, Database Management
        * Software, Human Resources
        * Software, Industrial IT & Others
        * Software, Job Costing/Estimating/Quoting
        * Software, Machinability Databases
        * Software, Product Life Cycle Management
        * Software, Quality Documentation
        * Software, Statistical Data Collection & Analysis
        * Software, Tool Crib Control
        * Software, Tool Design
        * Software, Virtual Reality Systems
        * Software, Administrative
    - Computational Object
        * Digitizing/Scanning Systems
    - Software Agent
        * Industrial Internet of Things (IIoT) Technology
        * Machine Monitoring Systems
        * Software, Automation
        * Software, ERP for Make-to-Order Manufacturing
        * Software, ERP for Standard Products Manufacturing
        * Software, Manufacturing Execution Systems (MES)
        * Energy Management Software, Systems & Services
        * Software, Production Floor        
    - Artificial Intelligence
        * Industrial Artificial Intelligence
            * Autonomous AI
            * Generative AI
            * Predictive AI
            * Vision AI
- Process Auxiliary
    - Machine Accessory
        * EDM Filtration Equipment and Supplies
        * Laser Accessories and Supplies
        * Ball/Lead Screws and Repair Services
        * Bearings, Pulleys, Clutches, Gears and V-Belts
        * Centers, Live and Other
        * Chillers
        * Compressors
        * Connectors & Fasteners
        * Couplings
        * Dividing and Indexing Heads
        * Drilling Heads/Attachments
        * Electrical Switchgear, Enclosures and Components
        * Electronic and Opto-Electronic Components
        * Feedback Devices (Encoders/Resolvers/Scales/Sensors)
        * Formed Parts (castings, sintered, etc.)
        * Grinding Attachments and Accessories
        * Grinding Wheel Dressing Units
        * Guide Bushings (for Swiss Lathes)
        * Hand Tools Of All Types
        * Handwheels, Knobs, Cranks, Etc.
        * Heat Exchangers and Air Conditioning Equipment
        * Hydraulic and Pneumatic Equipment
        * Indexers and Rotary Tables
        * Lighting Equipment
        * Lights, Indicator and Warning
        * Linear Motion Systems
        * Lubricants and Greases
        * Lubricators
        * Machine Castings and Weldments
        * Machinery Mountings, Levels, Anchors, Dampers
        * Magnets and Magnetic Devices of all Kinds
        * Milling Heads/Attachments
        * Motors, Non-Servo (Including Spindles, Drive Units, & Power Feeds)
        * Nameplates, Labels and Signs
        * Press Safety Equipment
        * Protective Enclosures and Covers
        * Pumps, Coolant
        * Replacement Parts/Repair Parts
        * Seals
        * Slides, Ways, and Linear Guides
        * Speed Reducers and Gear Boxes
        * Spindles
        * Springs
        * Steady Rests
        * Tool Condition Monitoring Systems
        * Tool Presetting Equipment
        * Tool Storage and Handling Systems
        * Tool Turrets
        * Way Covers, Way Protectors and Seals
        * Pallets, Tote Boxes and Separators
        * Servo Motors, Linear
        * Servo Motors, Rotary
        * Voltage/Current Regulators, Transformers and Phase Converters
        * Minimum Quantity Lubrication (MQL) Systems
        * Paint/Sealants/Adhesives
        * Toolchangers
    - Tooling
        * Abrasive Compounds and Slurries
        * Arbors (for Cutters)
        * Boring Tools & Heads for Machining Centers
        * Boring Tools for Turning Machines
        * Boring Tools/Bars/Heads
        * Broaching Tools
        * Buffing and Polishing Supplies
        * Burnishing Tools (Roller)
        * Burnishing, Honing and Lapping Tools
        * Carbide Tooling & Coatings
        * Chamfering Tools
        * Collets, Solid and Master
        * Counterbores/Countersinks
        * Cut-Off Tools/Attachments
        * Deburring Tools
        * Diamond Tooling, Coating, Abrasive, Single-point
        * Drill Bushings
        * Drill Chucks
        * Drills
        * End Mills
        * Facing Tools/Heads
        * Form Tools
        * Gear Cutting Tools
        * Gear Rolling Tools
        * Grinding Wheels and Abrasive Belts
        * Grooving Tools
        * Gundrills
        * Inserts, Indexable (Carbide, etc.) and Tool Inserts
        * Key Seating Tools
        * Knurling Tools
        * Marking Tools
        * Mass Finishing Media and Compounds
        * Milling Cutters
        * Protective Sleeves and Coatings (for Cutting Tools)
        * Punching Tools/Dies
        * Reamers
        * Saw Blades
        * Spline Rolling Tools
        * Tap Drivers and Attachments
        * Taps
        * Threading Tools - Cutting
        * Threading Tools - Forming
        * Tool Blanks
        * Tool Conditioning Equipment
        * Tool Holders and Adapters
        * Tool Holders and Adapters - Hydraulic
        * Tooling Systems, Modular and/or Quick-Change
        * Shear Blades
        * Ultrasonic Tooling
        * EDM Tooling
    - Workholding
        * Robot Tooling/Grippers
        - Chuck
            * Chucks, Collet Type
            * Chucks, Diaphragm
            * Chucks, Gear
            * Chucks, Index
            * Chucks, Jaw Type
            * Chucks, Magnetic
            * Chucks, Power
            * Chucks, Precision
            * Chucks, Self-Contained
            * Chucks, Special
            * Chucks, Vacuum
        * Pallet Changers - Automatic
        * Actuators, Hydraulic and Pneumatic
        * Angle & Sub Plates
        * Arbors, Expandable Workholding
        * Chuck Jaws and Collets
        * Clamps and Fixturing Devices
        * Fixturing Systems
        * Mandrels
        * Pedestal Type Fixtures and Tombstone Blocks
        * Vises and Vise Jaws
        * Workholding, Custom
- Business Offering
    * Compressed Air & Vacuum Technology Services
    * Machinery Rigging, Erection and Installation Services
    * Recruitment and Placement Services
    * Shipping & Logistics Services
    * Support and Maintenance Services ( for Drives, Gears Hydraulics, Pneumatics)
    * University, College or Research Center
    * Automation Services
    * Books, Publications and Information Services
    * Consulting Services
    * Contract Manufacturing and Machining Services
    * Deburring Services
    * Engineering Services (for Drives, Gears, Hydraulics, Pneumatics)
    * Finishing Services
    * Industrial IT Services & Outsourcing
    * Leasing and Financing Services
    * Machining Services
    * Maintenance and Repair Services
    * Metal Forming Services (solid forming, sheet metal forming & other formed metal parts)
    * Metal Sales & Services
    * Quality & Measurement Services
    * Reclamation/Recycling Services, Scrap Dealers
    * Retrofitting, Rebuilding, Remanufacturing Services
    * Service
    * Services, Assemblies & Metal Structures (springs, sheet metal & wire parts)
    * Services, Non-Metal Parts (plastic, rubber, ceramic)
    * Services, Sales
    * Spindle Repair and Maintenance Services
    * Tool Repair, Coating, and Treatment Services
    * Trade Associations, Societies, Governmental Agencies
    * Training of All Types, Including Audiovisual
    * Appraisal Services
    * Request for Quote Services, Internet-Only
    * Insurance/Financial/HR Services
    * Prototyping or Design Services
- Material
    * Electrode Materials, EDM Wire, and Supplies
    * Proofing Materials  (Wax, etc.)
    * Ferrous Materials
    * Non-Ferrous Materials
    * Tooling Materials, Blanks and Pre-Machined Stock of All Types
    * Tooling Materials, Carbides
    * Tooling Materials, Ceramics
    * Tooling Materials, Other
    * Workpiece Materials and Stock of All Kinds
    * Intelligent Materials, Smart Materials
